 ...
 *   npx tsx scripts/merge-leads-enterprise.ts --execute
 *
 * 可选环境变量（默认与计划一致）：
 *   MERGE_LEADS_ENT_SRC=jcqy-dy
 *   MERGE_LEADS_ENT_DST=7273749651499548709
 *
...
const SRC = process.env.MERGE_LEADS_ENT_SRC?.trim() ?? "jcqy-dy";
const DST = process.env.MERGE_LEADS_ENT_DST?.trim() ?? "7273749651499548709";
...
async function runMerge(tenantId: string, execute: boolean): Promise<void> {
  ...
  try {
    if (!execute) {
      log("[dry-run] 未执行写操作。加 --execute 提交事务。");
      return;
    }
